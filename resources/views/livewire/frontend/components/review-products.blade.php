...
<div class="col-lg-4 col-md-6">
    <div class="latest-product__text">
        ...
        <div class="latest-product__slider owl-carousel owl-loaded owl-drag">
            ...
            @for ($i = 0, $k=0; $i < count($review_products)/3; $i++)
                <div class="owl-item" style="width: 405.31px;">
                <div class="latest-prdouct__slider__item">
                    @for ($j = 0; $j <3; $j++,$k++)
                        @if(count($review_products)<=$k)
                            @break
                        @endif
                        <a href='{{url("product/".$review_products[$k]->slug)}}' class="latest-product__item">
                            <div class="latest-product__item__pic">
                                <img src="{{asset($review_products[$k]->featured_image)}}" alt="" >
                            </div>
                            <div class="latest-product__item__text">
                                <h6>{{$review_products[$k]->name}}</h6>
                                <span>{{product_settings()->currency}}&nbsp;{{$review_products[$k]->unit_price}}</span>
                            </div>
                        </a>
                    @endfor
                </div>
