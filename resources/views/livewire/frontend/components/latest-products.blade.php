...
<div>
        ...
        <div class="owl-stage-outer"><div class="owl-stage" style="transform: translate3d(-810px, 0px, 0px); transition: all 1.2s ease 0s; width: 2432px;">
            @for ($i = 0, $k=0; $i < count($latest_products)/3; $i++)
                <div class="owl-item" style="width: 405.31px;">
                <div class="latest-prdouct__slider__item">
                    @for ($j = 0; $j <3; $j++,$k++)
                        @if(count($latest_products)<=$k)
                            @break
                        @endif
                        <a href='{{url("product/".$latest_products[$k]->slug)}}'' class="latest-product__item">
                            <div class="latest-product__item__pic">
                                <img src="{{asset($latest_products[$k]->featured_image)}}" alt="" >
                            </div>
                            <div class="latest-product__item__text">
                                <h6>{{$latest_products[$k]->name}}</h6>
                                <span>{{product_settings()->currency}}&nbsp;{{$latest_products[$k]->unit_price}}</span>
                            </div>
                        </a>
